...
- The belt covers **two** channels — formatted message text (`Scrub`) and the
  exception object (`ScrubException`) — for categories under `PatchManagement.Vault.`
  only. Scope state is a known third channel, guarded by convention rather than
  filtering.
- Message-scrubbing is **inert in production today** and that is by design, not an
  unfinished task. The filter's live effect is the exception handling.
